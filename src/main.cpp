...
// MIDI library includes - serial MIDI, USB MIDI as a class-compliant device
// To compile for a USB MIDI enabled device like the Teensy, you will have to
// - (in the Arduino/Teensyduino IDE) select Tools->USB Type->MIDI, OR
// - (in the PlatformIO IDE) add a variable to the platformio.ini file: build_flags = -UUSB_SERIAL -DUSB_MIDI

#define HAS_USBMIDI TRUE

// Needs my OLED_I2C_128x64_Monochrome library to link:
...
const byte VERSION[] PROGMEM = "FW V0.6";

// V0.6: 
// - calibrate() routine extended to adjust for deadband and center
// - Fixed code to compile on PlatformIO IDE
// - Got rid of midiUSB special function; conditional compile
// V0.5:
...
void loop() {
  ...
  if (button1.update()  || RefreshFlag)
  {
    ...
    if (EEPROM[3] > 0x80) // Aftertouch?
    {
      ...
      #ifdef HAS_USBMIDI 
        midiB.sendAfterTouch(!button1.read()*EEPROM[19],MidiTxCh);   
      #endif  
    } else {
      midiA.sendControlChange(EEPROM[3],!button1.read() * 127,MidiTxCh);
      #ifdef HAS_USBMIDI 
        midiB.sendControlChange(EEPROM[3],!button1.read() * 127,MidiTxCh);
      #endif
    }
    oled.fontsize=1;
  ...
  if (button2.update()  || RefreshFlag)
  {
    ...
    if (EEPROM[4] > 0x80) // Aftertouch?
    {
      ...
      #ifdef HAS_USBMIDI 
        midiB.sendAfterTouch(!button2.read()*EEPROM[19],MidiTxCh); 
      #endif    
    } else {
      midiA.sendControlChange(EEPROM[4],button2.read() ? 127 : 0,MidiTxCh);
      #ifdef HAS_USBMIDI 
        midiB.sendControlChange(EEPROM[4],button2.read() ? 127 : 0,MidiTxCh);
      #endif
    }
    oled.fontsize=1;
  ...
  if (button3.update() || RefreshFlag)
  {
    ...
    if (EEPROM[5] > 0x80) // Aftertouch?
    {
      ...
      #ifdef HAS_USBMIDI 
        midiB.sendAfterTouch(!button3.read()*EEPROM[19],MidiTxCh);     
      #endif
    } else {
      midiA.sendControlChange(EEPROM[5],button3.read() ? 127 : 0,MidiTxCh);
      #ifdef HAS_USBMIDI 
        midiB.sendControlChange(EEPROM[5],button3.read() ? 127 : 0,MidiTxCh);
      #endif
    }
    oled.fontsize=1;
...
// ...but with the most recent version of the MIDI library, V4.3:
  ...
  if ((value_wheel_b != value)  || RefreshFlag)
  {
    ...
    // Ignore ATO for the moment
      ...
      #ifdef HAS_USBMIDI 
        midiB.sendControlChange(EEPROM[2] % 0x7f,value,MidiTxCh);  
      #endif
    }
